...
1. **Step 0の発掘結果を反映**し、新規企業を独占マップに追加
2. トリガーイベントに基づき既存企業を更新
3. **6ドメインのカバレッジバランスを維持**
...
| 条件 | 詳細 | 優先度 |
|------|------|--------|
| **Step 0発掘結果あり** | discovery_result_*.jsonに新規候補あり | **最優先** |
| トリガーイベント | Step 1で `is_trigger_for: "monopoly_map"` のイベントあり | 高 |
| ドメインカバレッジ不足 | 6ドメインのいずれかが0社 | 高 |
| 定期更新 | 前回更新から30日以上経過 | 中 |
| 強制更新 | ユーザーが「独占マップを更新」と指示 | - |
| 品質アラート | information_quality.jsonでシェア関連のstaleアラートあり | 中 |
...
```
1. data/discovery/discovery_result_YYYYMMDD.json を読み込み
2. candidates_discoveredの企業について:
   - recommendation が "追加" または "追加推奨" の企業を抽出
   - 4次元評価を確認（certainty.score >= 50, asymmetry.score >= 10）
   - monopoly_map.json の companies に追加
3. ドメインカバレッジを再計算
```
...
**⚠️ いずれかのドメインが0社の場合、Step 0の追加発掘を推奨**
