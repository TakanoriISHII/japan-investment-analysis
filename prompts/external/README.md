...
### データの受け渡し方法（企業リストファイル方式）

| タスク | 入力データ | 入力方法 |
|--------|-----------|----------|
| タスク1（企業発掘） | なし | そのまま実行 → **2ファイル出力** |
| タスク2（広域情報） | なし | そのまま実行 |
| タスク3（独占情報） | 企業リスト（推奨） | `company_list_*.md` を参照 |
| タスク4（財務情報） | **企業リスト（必須）** | `company_list_*.md` を参照 |

### タスク1の出力（2ファイル）

```
タスク1実行後:
├── data/intelligence/raw/company_discovery/[llm]_YYYYMMDD.md  ← 詳細分析（2000行）
└── data/intelligence/company_list_YYYYMMDD.md                 ← 企業リスト（軽量）
```

**タスク3/4は軽量な `company_list_*.md` を参照**（巨大な詳細ファイルではない）
...
3. **タスク3/4の場合**: `company_list_*.md` をアップロード
4. 外部LLMに貼り付けて実行
...
1. まずタスク1を実行 → company_list_YYYYMMDD.md が出力される
2. タスク3/4実行時に、その company_list ファイルをアップロード
3. LLMが50社を参照して情報収集を実行
```
