...
| コマンド | アクション |
|---------|----------|
| `approve top30` | Top 30選定を承認（意思決定者用） |
...
| 1-8 | シグナルトリガー確認 | signal_config.jsonの全トリガーを現在の市場データと照合し、active_signals.jsonを更新した【v3.0.0〜】 |
| 1-9 | 為替感応度確認 | currency_sensitivity.jsonのベースレートと現在レートの乖離を確認した【v3.0.0〜】 |
...
═══════════════════════════════════════════════════════════════════
9-1. 予測記録（パフォーマンス検証用）【必須】
═══════════════════════════════════════════════════════════════════

以下のファイルを data/performance/predictions/[TODAY]/ に保存:

┌────────────────────────────────────────────────────────────────┐
│ 【必須】top30_snapshot.json                                    │
│   - Japan Top 30全銘柄のスコア・予測                          │
│   - 各銘柄のkey_catalyst、1m/3m/6m予測                        │
│   - 検証スケジュール（1ヶ月/3ヶ月/6ヶ月/1年）                │
├────────────────────────────────────────────────────────────────┤
│ 【必須】market_risk_snapshot.json                              │
│   - 市場リスクスコアと予測                                    │
│   - シナリオ別予測（ベース/ブル/ベア）                        │
│   - ティア別予測                                              │
├────────────────────────────────────────────────────────────────┤
...
- state.json を更新
  - performance_verification セクションに予測記録を登録
  - approval_workflow でTop30承認状況を記録
  - escalation_log にエスカレーション履歴を追加（該当あれば）
- latest/ フォルダを更新
- 次回分析への引き継ぎ情報を記録
...
| # | 確認項目 | 確認内容 |
|---|---------|---------|
| 9-1 | 予測スナップショット | `data/performance/predictions/[DATE]/top30_snapshot.json` を作成した【必須】 |
| 9-2 | 市場リスク予測 | `data/performance/predictions/[DATE]/market_risk_snapshot.json` を作成した【必須】 |
| 9-3 | state.json更新 | `performance_verification` セクションを更新した |
| 9-4 | 承認ワークフロー | `approval_workflow` でTop30承認状況を記録した |
| 9-5 | latest更新 | `latest/` を最新レポートで更新した |
| 9-6 | 最終コミット | 全変更をコミットした |
...
`check status` または `state.json` で次回検証日を確認:

```json
"verification_schedule": {
  "1_month": {"date": "2026-02-28", "status": "pending"},
  "3_months": {"date": "2026-04-29", "status": "pending"},
  "6_months": {"date": "2026-07-29", "status": "pending"},
  "1_year": {"date": "2027-01-29", "status": "pending"}
}
```

### verify performance 実行手順

```
┌─────────────────────────────────────────────────────────────────┐
│            verify performance [period] 実行フロー               │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. 予測スナップショットを読み込み                              │
│     → data/performance/predictions/[SNAPSHOT_DATE]/             │
│                                                                 │
│  2. 現在の実績データを収集                                      │
│     → 株価（Yahoo Finance等）                                   │
│     → カタリスト発生状況                                        │
...
│     ┌───────────────────────────────────────────────────┐      │
│     │ 【1ヶ月検証】                                      │      │
│     │ - Top 30の株価パフォーマンス（vs TOPIX）           │      │
│     │ - 予測カタリストの発生率（目標: 50%+）            │      │
│     │ - 市場リスク予測の的中度                          │      │
│     ├───────────────────────────────────────────────────┤      │
│     │ 【3ヶ月検証】                                      │      │
│     │ - 非対称性解消率（高非対称性銘柄の認識変化）      │      │
│     │ - 5軸スコア上位/下位銘柄のリターン差              │      │
│     │ - シナリオ予測の的中度                            │      │
│     ├───────────────────────────────────────────────────┤      │
│     │ 【6ヶ月検証】                                      │      │
│     │ - 高確実性銘柄の進捗状況（目標: 70%+想定通り）    │      │
│     │ - 象限別パフォーマンス（Prime vs Others）         │      │
│     │ - 軸別の予測力（相関分析）                        │      │
│     ├───────────────────────────────────────────────────┤      │
│     │ 【1年検証】                                        │      │
│     │ - フレームワーク総合評価                          │      │
...
│                                                                 │
│  4. 検証レポートを生成                                          │
│     → data/performance/analysis/[period]_review.json            │
│     → reports/[DATE]/performance_verification.md               │
│                                                                 │
│  5. state.json を更新                                           │
│     → verification_schedule.[period].status = "completed"       │
│     → last_verified = [TODAY]                                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 期間 | 指標 | 成功基準 | 失敗時アクション |
|------|------|---------|----------------|
| 1ヶ月 | カタリスト発生率 | 50%+ | カタリスト予測精度を見直し |
| 1ヶ月 | Top 30 vs TOPIX | アウトパフォーム | 銘柄選定基準を見直し |
| 3ヶ月 | 非対称性解消率 | 20%+ | 非対称性評価を厳格化 |
| 3ヶ月 | 軸上位/下位差 | 5%+ | 軸の重み付けを調整 |
| 6ヶ月 | 高確実性的中率 | 70%+ | 確実性評価基準を見直し |
| 1年 | 年間超過リターン | TOPIX+5%+ | フレームワーク全面改訂 |
...
─────────────────────────────────────────────────────────
相関係数 ≥ 0.4 → 有効（現状維持）
相関係数 0.2-0.4 → 要観察（次回も継続確認）
相関係数 < 0.2 → 無効（重み付け削減を検討）
─────────────────────────────────────────────────────────
...
以下の条件を満たす場合、`escalation_log` に自動記録し、意思決定者への通知を推奨:
...
│  【市場リスク関連】                                              │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │ 条件1: 市場リスクスコア ≥ 76（極端リスク）                │ │
│  │ → アクション: ポジション縮小を推奨、意思決定者承認必須    │ │
│  │                                                           │ │
│  │ 条件2: 市場リスク変動 ≥ ±15（1週間以内）                  │ │
│  │ → アクション: 緊急レビュー、ポジション再評価              │ │
│  │                                                           │ │
│  │ 条件3: ティア1指標が閾値超過                              │ │
│  │   - 日銀政策転換シグナル（利上げ示唆、YCC修正）          │ │
│  │   - 日経平均PER > 20 または PBR > 1.6                     │ │
│  │ → アクション: 意思決定者への即時報告                      │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
│  【Top 30関連】                                                  │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │ 条件4: Top 5銘柄のスコア変動 ≥ ±8                         │ │
│  │ → アクション: 変動理由の詳細分析、承認レビュー            │ │
│  │                                                           │ │
│  │ 条件5: Top 10の入れ替え ≥ 3銘柄（1回の分析で）            │ │
│  │ → アクション: 大幅変動の妥当性確認、承認必須              │ │
│  │                                                           │ │
│  │ 条件6: 重大な矛盾が未解消（Top 10銘柄）                   │ │
│  │ → アクション: 矛盾解消まで当該銘柄の推奨保留              │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
│  【情報品質関連】                                                │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │ 条件7: 情報品質スコア < 80                                 │ │
│  │ → アクション: 分析結果の信頼性警告、追加検証推奨          │ │
│  │                                                           │ │
│  │ 条件8: 3LLM一致率 < 50%                                    │ │
│  │ → アクション: 情報収集プロセスの見直し                    │ │
│  └───────────────────────────────────────────────────────────┘ │
...
│  【地政学関連】                                                  │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │ 条件9: 台湾海峡リスク評価が「🟠高リスク」に変化           │ │
│  │ → アクション: 半導体関連ポジションの緊急レビュー          │ │
│  │                                                           │ │
│  │ 条件10: 日中関係が「🟠高緊張」に変化                       │ │
│  │ → アクション: 中国売上比率の高い銘柄を再評価              │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
  ...
  "escalation_log": [
    {
      ...
      "condition": "market_risk_extreme",
      "trigger_value": 78,
      "threshold": 76,
      "severity": "critical",
      "message": "市場リスクスコアが76を超過（78）。ディフェンシブスタンスを推奨。",
      "recommended_action": "キャッシュ比率30%以上、投機的ポジション解消",
      "acknowledged": false,
...
│  ───────────────────────────────────────────────────────────── │
│  条件: 以下が維持される場合                                      │
│  - 日銀: 現状維持〜年1回の小幅利上げ（0.25%以内）               │
│  - 円ドル: 140-160円のレンジ内                                   │
│  - 日経平均: ±15%以内の変動                                      │
│  - 地政学: 現状の緊張度維持（急激な悪化/改善なし）               │
│                                                                 │
│  【ブルシナリオ】（確率: 20-30%）                                │
│  ───────────────────────────────────────────────────────────── │
│  トリガー（いずれか発生で移行）:                                  │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │ 金融: 日銀が利上げ見送り継続（年内ゼロ回）                │ │
│  │ 為替: 円安加速（165円以上）→ 輸出企業に追い風            │ │
│  │ 政策: GX投資/防衛費の追加増額決定                         │ │
│  │ 地政学: 米中関係改善、台湾海峡緊張緩和                    │ │
│  │ 企業業績: 主要企業がガイダンス上方修正（5社以上）         │ │
│  └───────────────────────────────────────────────────────────┘ │
│  Top 30への影響:                                                 │
│  - 高シナジー銘柄（軸3高スコア）が恩恵                          │
│  - 投機的象限の銘柄がアウトパフォーム                           │
...
│  トリガー（いずれか発生で移行）:                                  │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │ 金融: 日銀が予想外の利上げ（0.5%以上の一度の利上げ）      │ │
│  │ 為替: 急激な円高（130円以下）→ 輸出企業に逆風            │ │
│  │ バリュエーション: 日経平均PER > 22 からの調整             │ │
│  │ 地政学: 台湾海峡で軍事的緊張激化                          │ │
│  │ 外部ショック: 米国景気後退確定、中国経済危機              │ │
│  │ 企業業績: 主要企業がガイダンス下方修正（5社以上）         │ │
│  └───────────────────────────────────────────────────────────┘ │
│  Top 30への影響:                                                 │
│  - 高独占力・高耐性銘柄（軸1・2高スコア）が相対的に堅調        │
│  - 投機的象限の銘柄を除外                                       │
│  - 市場リスクスコア: +15〜+25の調整                             │
│  - キャッシュ比率を20%以上に引き上げ推奨                        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 指標 | ベース範囲 | ブル移行 | ベア移行 | データソース |
|------|-----------|---------|---------|-------------|
| 日銀政策金利 | 0.5-1.0% | 据え置き継続 | 0.5%以上利上げ | 日銀会合 |
| ドル円 | 140-160円 | 165円以上 | 130円以下 | Bloomberg |
| 日経平均PER | 14-18倍 | 18倍維持で上昇 | 22倍超からの急落 | 日経新聞 |
| 日経VI | 20-30 | 18以下 | 35以上 | 大阪取引所 |
| 海外投資家動向 | 小幅売買 | 3週連続買越 | 3週連続売越 | 東証データ |
| GX/防衛予算 | 予定通り | 追加増額 | 削減/凍結 | 閣議決定 |
...
シナリオ移行検知時:
─────────────────────────────────────────────────────────
1. state.json の scenario_status を更新
2. エスカレーションログに記録
3. market_risk.json のシナリオ確率を更新
4. Top 30の象限戦略を再計算
5. レポートに「シナリオ移行アラート」を追加
─────────────────────────────────────────────────────────
```
